...
if (!(Test-Path .\scaledemotemplate.json)) 
...
else 
{
	...
	#Random
	$Random1 = (Get-Random -Minimum 10000 -Maximum 99999)
	$Random2 = (Get-Random -Minimum 10000 -Maximum 99999)
	$TM_URL = "scaleDemo"  + (Get-Random -Minimum 10000 -Maximum 99999) + ".trafficmanager.net"

	#Global Settings
	$Websites_API_Version = "2014-04-01"

	#Resource Group Properties
	$RG_Name = "scaleDemo"
	$RG_Location = "West Us"

	#WebHosting Location1
	$WHP1_Name = "WHP" + $Random1
	$WHP1_Location = "West Europe"

	#WebHosting Location2
	...
	New-AzureResourceGroup -name $RG_Name -location $RG_Location -TemplateFile .\scaledemotemplate.json -whp1 $WHP1_Name -location1 $WHP1_Location -siteName1 $WS1_Name -whp2 $WHP2_Name -location2 $WHP2_Location -siteName2 $WS2_Name -serverName1 $SQL1_Server -serverName2 $SQL2_Server
	...
	$Site2Settings = @{ "dbRead" = "read"; "dbWrite" = "write"; "SiteName" = $WS2_Name; "queueName"="scaledemo"; "blobContainer" = "scaledemo" }
	...
	#Execute Script 
	$SQLCommandText = @(Get-Content -Path "./script.sql") 
